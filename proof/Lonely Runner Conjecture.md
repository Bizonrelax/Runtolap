...
$$ \operatorname{dist}\Bigl(\{t/a\},\,\{t/b_i\}\Bigr) \;\ge\; \delta $$
where $\operatorname{dist}(x,y)=\min\bigl(|x-y|,\,1-|x-y|\bigr)$, and $\{x\}$ denotes the fractional part of $x$.
...
$$ L = \operatorname{lcm}(C) $$

Consider the discrete time moments $t = kL \; (k \in \mathbb{N})$. At these specific moments, for any runner $b_i \in C$, we have $\{t/b_i\}=0$, meaning all runners belonging to the subset $C$ are synchronized exactly at the starting position. The phase of our target runner $A$ at these moments is given by the sequence:
$$ \varphi_A(k) = \Bigl\{\frac{kL}{a}\Bigr\} $$

**Lemma 1.** If $a$ does not divide $L$, then there exists an integer $k$ such that:
$$ \operatorname{dist}\bigl(\varphi_A(k), 0\bigr) \ge \delta $$

*Proof.* Let $d = \gcd(a, L)$. Then the fraction $a/d$ represents the exact period of the sequence $\varphi_A(k)$. By the initial assumption, $a \nmid L \implies a/d > 1$. As $k$ varies, the sequence visits $a/d$ uniformly distributed points on the unit circle, including $0$ and $1/(a/d)$. Since $a/d \ge 2$ and $\delta = 1/n \le 1/2$, the circle must contain at least one point whose distance to $0$ is greater than or equal to $\delta$. (If $a/d=2$, the point $1/2$ satisfies the condition; if $a/d > 2$, the condition holds naturally). Selecting the corresponding index $k$ completes the proof of Lemma 1.
...
### Case 1: $\operatorname{lcm}(B)$ is not a multiple of $a$
...
### Case 2: $\operatorname{lcm}(B)$ is a multiple of $a$
Since all $b_i$ are distinct integers, we isolate a minimal (by inclusion) subset $C \subseteq B$ such that $\operatorname{lcm}(C)$ is a multiple of $a$. Minimality implies that for any proper subset $C' \subset C$, the period $a$ does **not** divide $\operatorname{lcm}(C')$. This structure triggers our main inductive framework.
...
Let the relative velocities with respect to runner $A$ be defined as $v_i = \frac{1}{b_i} - \frac{1}{a}$ for $i = 1, \dots, n-1$. We define the rank of the system as:
$$ r = \dim_{\mathbb{Q}} \operatorname{span}_{\mathbb{Q}}(v_1, v_2, \dots, v_{n-1}) $$
...
The closure of the phase trajectory forms an $(r+1)$-dimensional subtorus $\mathbb{T}^{r+1} \subseteq \mathbb{T}^{n-1}$. We partition the velocity vector into a maximal linearly independent subset $C'$ ($\dim_{\mathbb{Q}}(C') = r$) and a dependent layer $D$. Thus, there exists an integer matrix $M$ such that the phase evolution complies with the linear toral endomorphism:
$$ \vec{\varphi}_D(t) = M \cdot \vec{\varphi}_C'(t) \pmod{\mathbb{Z}^{|D|}} $$
...
$$ H_j = \{ \vec{\theta} \in \mathbb{T}^{r+1} : \operatorname{dist}((M\vec{\theta})_j, 0) < \delta \} $$
...
When $r=1$, all periods $b_i$ divide $a$, and $\operatorname{lcm}(B) = a$. Let us isolate one runner $b_1 \in B$ and evaluate the remaining sub-hierarchy $C = B \setminus \{b_1\}$. Let $L = \operatorname{lcm}(C)$. We analyze the system over discrete time steps $t = mL$, mapping the evolution to the residue ring $\mathbb{Z}_p$, where $p = a/L \ge 2$.
...
Given that $k < \frac{p}{n}$, for any system with $n \ge 5$, the inequality $p \ge 4\left(\frac{p}{n}\right) + 3$ holds for all sufficiently large scaling factors $p$.
...
*   **For $n=3$ and $p=4$:** The modular system over $\mathbb{Z}_4$ yields a total collision only when $c=3$. This algebraic configuration implies $a = 4b_2$ and $b_2 = 3b_1$, forcing $a = 12b_1$. However, this structure implies $\operatorname{lcm}(b_1, b_2) = 3b_1 = a/4 < a$. This directly violates the core assumption of the Special Case ($\operatorname{lcm}(B) = a$). Such a configuration is blocked by the global reduction step (Case 1 in Section 3) and cannot appear in the cyclic base. For all valid parameters, a solution exists.
